...
struct token
{
  ...
  // TRUE = whitespace between token and next token
  bool whitespace;

  // pointer to the start of the bracket the token is between
  // for debugging
...
struct lex_process
{
  ...
  /**
   * Count of how many brackets there are
   * ((50)) == count 2
   */
  ...
  // this will be private data the lexer does not understand, but the person using the lexer does
  void *private;
};
...
struct node
{
  ...
  struct node_binded
  {
    // pointer to body node this is in
    struct node *owner;
